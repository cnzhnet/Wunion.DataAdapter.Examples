...
namespace Wunion.DataAdapter.CodeFirstDemo.Services
{
    ...
    /// 应用程序的用户账户服务.
    /// </summary>
    public class UserAccountService : AppService<UserAccount>
    {
        /// <summary>
        /// 创建一个应用程序的用户账户服务.
        /// </summary>
        ...
        public override void Add(UserAccount data)
        {
            using (BatchCommander batch = new BatchCommander(db.DbEngine))
            {
                ...
                Add<UserAccount>(db.UserAccounts, data, batch);
            }
        }
        ...
        public override void Update(UserAccount data)
        {
            using (BatchCommander batch = new BatchCommander(db.DbEngine)) 
            {
                ...
                Update<UserAccount>(db.UserAccounts, data, batch);
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<List<UserDataModel>> ListAsync()
        {
            List<UserDataModel> queryResult = await QueryBuilder<UserAccountDao>.Create(db.UserAccounts)
                .Select(p => p.First.All)
                .Build((p, options) => { options.OrderBy(p.First.UID, OrderByMode.ASC); })
                .ToListAsyn<UserDataModel>();
            return queryResult;
        }
        ...
        /// <returns></returns>
        public async Task<UserAuthorization> LogInAsync(string name, string password)
        {
            UserAuthorization authorization = await Task.Run<UserAuthorization>(() => {
                UserAccount account;
                ...
                return new UserAuthorization { UID = account.UID, Permissions = permissions.Distinct().ToList() };
            });
